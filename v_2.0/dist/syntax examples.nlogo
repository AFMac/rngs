...
to setup
  ;rngs extension needs to be initialized before first use
  print "Initialization complete."
  rngs:init
end

to go
  ;individual random number streams can be used, each with their own seed
  ;syntax is -->  rngs:set-seed <stream id> <seed>
  ;both <stream id> and <seed> need to be integers
  rngs:set-seed 1 100
  rngs:set-seed 2 50

  ;Exponential distribution
  ;syntax --> rngs:rnd-exponential <stream id> <lambda>
  ;lambda can be non-integer (a double), but must be > 0
  print word "Exponential:" rngs:rnd-exponential 1 5
  ...
  ;    - alpha = (x*x)/y
  ;    - lambda = 1/(y/x)
  print word "Gamma: " rngs:rnd-gamma 1 .5 3.2
